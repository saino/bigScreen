...
var TouchSlide = function(a){
	...
	var doSwitchLoad=function( moving ){ 
				...
				var changeImg = function( ind ){
						//排除拉出边界
						if(ind < 0 || ind >= conBoxSize){
							return false;
						}
                    //update by guYY 2016/1/22 16:52
						var img = [];
                        if(conBox && conBox.children[ind])
                        {
	...
	var doPlay=function(isTouch){
        //update by guYY 2015/10/21
		if(!changeAuto) {
            clearInterval(inter);
            inter = null;
        }
        if(!changeAuto)return;
        switch (effect)
		{
			case "left": 
	...
	var tMove = function(e){
        ...
        //modify by fishYu 2016-3-8 9:50 控制banner左右滑动的时候，不触发下拉
